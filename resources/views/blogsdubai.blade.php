...
@endsection
@section('title', 'Blog de Viajes a Dubai')

@section('meta')
	<meta name="title" content="Blog de viajes a Dubai">
	<meta name="keywords" content="Blog de viajes a Dubai">
	<meta name="description" content="Aquí le hemos recupilado los mejores blogs de viajes a Dubai para que te ayuden a organizar a tu viaje. ¡Haz Clic para saber más información sobre Dubai!.">
@endsection
@section('content')

{{-- <section class="parallax-window" data-parallax="scroll" data-image-src="{{ asset('/images/dubai.jpeg')}}" data-natural-width="1400" data-natural-height="470">
    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-lg-7 col-sm-6 nopadding">
                @if ($dblogs[0])
                    {{-- <p>{{ $dblogs[0]->title }}</p> --}}
                    <a href="/blog-de-viajes/blog-de-viajes-a-dubai/{{$dblogs[0]->slug}}">
                        <div class="item big" style="background-image: url({{ asset('/images/blogs/'.$dblogs[0]->image)}})">
                            <div class="overlay">
                                <div class="content">
                                    <h3>{{ $dblogs[0]->title }}</h3>
                                    <h6><span>{{$dblogs[0]->created_at->format('d F')}}</span></h6>
                                </div>
                            </div>
                        </div>
                    </a>
            ...
            <div class="col-lg-5 col-sm-6 nopadding">
                <div class="row nomargin">
                    @if (Arr::except($dblogs, 0))
                        @foreach ($dblogs as $dblog)
                            <div class="col-12 nopadding pl-sm-3">
                                <a href="/blog-de-viajes/blog-de-viajes-a-dubai/{{$dblog->slug}}">
                                    <div class="item" style="background-image: url({{ asset('/images/blogs/'.$dblog->image)}})">
                                        <div class="overlay">
                                            <div class="content">
                                                <h3>{{ $dblog->title }}</h3>
                                                <h6><span>{{$dblog->created_at->format('d F')}}</span></h6>
                                            </div>
                                        </div>
                                    </div>
                                </a>
...
{{-- 
    ...
    <div class="featured">
        <div class="container">
            ...
            <div class="details">
                <div class="row">
                    @foreach ($dblogss as $dblog)
                    <div class="col-md-3">
                        <div class="box">
                            <a href="/blog-de-viajes/blog-de-viajes-a-dubai/{{ $dblog->slug }}">
                                <div class="pic" style="background-image: url('{{ asset('/images/blogs/'.$dblog->image) }}')">
                                </div>
                            </a>
                            <div class="content">
                                <a href="/{{ $dblog->slug }}">
                            ...
                            <div class="post">
                                <div class="row nomargin">
                                    <div class="col-lg-4 col-md-5 nopadding">
                                        <a hreflang="es" href="/blog-de-viajes/blog-de-viajes-a-dubai/{{$blog->slug}}"><img src="{{ asset('/images/blogs/'.$blog->image)}}" title="{{$blog->imageTitle}}" alt="Image" class="img-fluid">
                                        </a>
                                    </div>
                                    <div class="col-lg-8 col-md-7 nopadding">
                                        <div class="contant">
                                            <a hreflang="es" href="/blog-de-viajes/blog-de-viajes-a-dubai/{{$blog->slug}}"><h3>{{$blog->title}}</h3></a>
                                            <h6><i class="icon-calendar-empty"></i><span>{{$blog->created_at->format(' Y/m/d')}}</span></h6>
                                            <p>
                                                {!!$blog->shortDescription!!}
                                            </p>
                                            {{-- <div class="text-right">
                                                <a hreflang="es" href="/blog-de-viajes/blog-de-viajes-a-dubai/{{$blog->slug}}" class="btn btn-primary">Read more</a>
                                            </div> --}}
                                        </div>
                                    </div>
                                </div>
