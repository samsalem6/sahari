...
@section('title', __('Edit Hotels'))
@section('content')

<div class="notes-env">
    <div class="notes-header">
        <h2>@lang('Edit Hotels')</h2>
        <div class="right">
        </div>
    </div>
</div>
...
<div class="container">
    <form  role="form" method="POST" action="{{action('HotelsController@update', $hotel->id)}}" enctype="multipart/form-data" class="form-horizontal form-groups-bordered">
        {{ csrf_field() }}
        @method('PUT')
        <div class="form-group {{ $errors->has('name') ? ' has-error validate-has-error' : '' }} ">
            <label for="name" class="col-sm-2 control-label">Name</label>
            <div class="col-sm-8">
                <input name="name" id="name" type="text" class="form-control" value="{{ $hotel->name }}">
        ...
        <div class="form-group {{ $errors->has('stars') ? ' has-error validate-has-error' : '' }} ">
            ...
            <div class="col-sm-8">
                <input name="stars" id="stars" type="text" class="form-control" value="{{ $hotel->stars }}">
        ...
        <div class="form-group {{ $errors->has('link') ? ' has-error validate-has-error' : '' }} ">
            ...
            <div class="col-sm-8">
                <input name="link" id="link" type="text" class="form-control" value="{{ $hotel->link }}">
