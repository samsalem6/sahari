        ...
        <div class="col-md-12">
            <div class="panel panel-primary">
                <div class="panel-body">
                    <form role="form" class="form-horizontal form-groups-bordered" method="POST" action="/admin/relatedPackage/store">
                        @csrf
                            ...
                            <div class="col-sm-8">
                                <select required name="Package_id" class="form-control">
                                    @foreach ($packages as $item)
                                        @if (itemRelatedblog($blog_id,$item->id))
                                        @else
                ...
                @if(count($Related) > 0)
                    @foreach($Related as $Relate)
                        <tr>
                            ...
                            <td style="text-align:center ">{{(int)Str::after($Relate->order,$blog_id)}}</td>
                            <td style="text-align:center">{{ $Relate->created_at->format('Y-m-d H:i A')}}</td>
                            <td style="text-align:center; width:40%">

                            <div class="col-sm-3">
                                <a href="/admin/relatedPackage/destroy/{{$Relate['id']}}"  class="btn btn-red btn-icon icon-left">
                                    Delete
