...
@section('content')
    ...
    <hr>
    <form role="form" method="POST" action="{{action('FaqController@update', $faq)}}" enctype="multipart/form-data" class="form-horizontal form-groups-bordered">
        {{ csrf_field() }}
        ...
        <div class="form-group {{ $errors->has('viewInHome') ? ' has-error validate-has-error' : '' }} ">
            ...
            <div class="col-sm-8">
                <input type="checkbox" class="form-check-input" {{ $faq->viewInHome == true ? 'checked' : '' }} id="viewInHome" name="viewInHome">
            ...
            <div class="col-sm-8">
                <select name="destination_id" class="form-control">
                    ...
                    @foreach ($destinations as $destination)
                        <option {{ $destination->id === $faq->destination_id ? "selected" : "" }} value="{{$destination->id}}">{{$destination->name}}</option>
                    @endforeach
                </select>
        ...
        <div class="form-group {{ $errors->has('body') ? ' has-error validate-has-error' : '' }} ">
            ...
            <div class="col-sm-8">
                <textarea data-stylesheet-url="/admin/css/wysihtml5-color.css" name="body" id="body" type="text" class=" form-control" id="field-6" >{{ $faq->body }}</textarea>
