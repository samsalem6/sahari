...
@endsection
@section('title')
{{$tdestinations->metaTitle}}
@endsection
@section('meta')
	<meta name="title" content="{{$tdestinations->metaTitle}}">
	<meta name="keywords" content="{{$tdestinations->metaKeywords}}">
	<meta name="description" content="{{$tdestinations->metaDescription}}">
@endsection
@section('content')

    @if ($tdestinations->type == 'video')
    <main>
        <section class="header-video">
        <img  src="" alt="Image" class="header-video--media" data-video-src="{{$tdestinations->video}}" data-teaser-source="video/paris" data-provider="youtube" data-video-width="854" data-video-height="480">
        </section>
    @else
    <div class="banners" style="background-image: url('{{ asset('/images/packages/'.$tdestinations->image)}}')">
        {{-- <div> --}}
                {{-- <div class="d-lg-none">
	...
	<div class="container margin_60">
		...
		<div class="row">
						...
						<div class="row nomargin">
								<div class="col-lg-4 col-md-4 nopadding">
                                    ...
                                    <a href="/{{ $package->slug }}">
										...
										{{-- <img width="100%" height="auto" src="url({{ asset('/images/packages/'.$package->shortImage)}})" title="{{ $package->altShort }}"> --}}
                                    </a>
									 ...
									 <div class="row">
										 <div class="col-md-8" >
											...
											@foreach ($package->icons as $index => $item)
												@if($index < 4)
													<li><a  href="#"><i style="color: #000" class="{{$item->icon}}"></i>{{$item->title}}</a>
													</li>
												@endif
											@endforeach
										</ul>
										<div class="rating">
										 ...
										 <div class="col-md-4">
											<div class="price_list text-right">
												<h6>{{Helper::currency_converter($package->startPrice)}}</h6>
												 <a href="/{{ $package->slug }}" class="button">Details</a>
				...
				<aside class="col-lg-4" id="sidebar">
					...
					<div class="theiaStickySidebar">
						<div class="box_style_1 expose" id="booking_box">
							<h3 style="background-color: #00aeff;" class="inner">Custom Tour</h3>
							...
							<form action="/tailor" method="post">
								...
								<div class="row ">
									...
									<div class="col-sm-6">
										...
										oninvalid="this.setCustomValidity('this field is required')"
										oninput="setCustomValidity('')"
											 name="Departure"   class="date-pick2 form-control {{ $errors->has('Departure') ? ' is-invalid' : '' }}" data-date-format="d/m/yyyy" placeholder="d/m/aaaa" type="text">
										</div>
									</div>
									...
									{{-- <div class="form-group col-md-6 col-sm-6 col-xs-12">
										<select name="TourType" class="form-control {{ $errors->has('TourType') ? ' is-invalid' : '' }}">
											...
											<option value="Egipto Jordania Y Israel">Egipto Jordania Y Israel</option>
									...
									<div class="col-sm-7">
										<div class="form-group">
											<input required=""
											oninvalid="this.setCustomValidity('this field is required')"
											oninput="setCustomValidity('')" value="{{old('whats')}}" type="text" class="form-control {{ $errors->has('whats') ? ' is-invalid' : '' }}" id="whats" name="whats" placeholder="Whats App Number *">
										</div>
									</div>
								...
								<div class="row">
									...
									<div class="col-6 ">
										<label style="font-size: 15px;">Children * ( 0 a 12 )</label>
										<div class="input-group">
								...
								<div class="form-group">
                                    ...
                                    oninvalid="this.setCustomValidity('this field is required')"
                                    oninput="setCustomValidity('')"
                                    type="text" class="form-control" id="comment" name="comment" placeholder="Please describe your tour
									requirements in order to help your travel consultant design the tour that ideally matches with you
									* What kinds of activities do you like?
									* Are there any places you’ve heard of and want to see?
									* Tell the specialist a bit about yourself"></textarea>
								</div>

								<button type="submit" style="width: 100%" class="btn btn-primary">INQUIRE NOW!</button>
							</form>
						</div>
						<!--/box_style_1 -->
					</div>
			...
			@endif
		           ...
		           @php
						$wikisss = \App\Model\Wiki::where('viewInSite',true)->where('destination_id', $tdestinations->id)->get();
					@endphp
		@if (count($wikisss) > 0)
		<div class="wiki">
			<div class="container">
				<h4 class="text-center">TRAVEL GUIDE</h4>
				<div class="row">
					
					@foreach ($wikisss as $wiki)
						<div class="col-md-4">
							<div class="box">
								<a href="/wiki/{{ $wiki->slug }}">
									<div class="pic" style="background-image: url({{ asset('/images/wikis/'.$wiki->image) }})"></div>
								</a>
								<div class="con">
									<a href="/wiki/{{ $wiki->slug }}">
										<h4>{{ $wiki->title }}</h4>
									</a>
									<p>{{ $wiki->shortDescription }}</p>
									<div class="text-right"><a href="/wiki/{{ $wiki->slug }}"><span class="more">Read More</span></a></div>
								</div>
							</div>
						</div>
					@endforeach
				...
				<div class="buttons text-center">
					<a href="/wiki" class="btn-main">View All</a>
				</div>
			</div>
		</div>
		...
		<div class="container">
			<div class="row">
				<div class="col-sm-5 mb-2">
					<img src="{{ asset('/images/packages/'.$tdestinations->faq_image)}}" alt="" height="auto" width="100%" style="border-radius: 5px;">
				</div>
				<div class="col-lg-7" id="faq">
		
					<div id="payment" class="accordion_styled">
						<h5 class="nomargin_top title">Frequently Questions</h5>
						  @foreach ($faqsDest as $faqs)
							<div class="card">
					...
					<p class="text-left nopadding">
						<a href="/faq" class="btn-main">View All</a>
					</p>
